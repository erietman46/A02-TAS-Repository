...
v4 core changes (targeting the consistent motion bias and V accuracy)
═══════════════════════════════════════════════════════════════════════
BIAS FIX (primary)
  1. Dynamic per-fold class weights in CrossEntropyLoss.
     For every training split, we count actual window counts for class 0
     and class 1 and compute weight_c = n_total / (2 * n_c).
     This neutralises window-level class imbalance (the direct cause of
     the consistent +0.133 motion bias across all three vehicles).
     Focal loss and label smoothing are removed — they amplified the bias
     rather than fixing it because they concentrated gradient mass on
     uncertain examples which were already skewed toward motion.

V ACCURACY FIX (secondary)
  2. Constrained feature set for the inner-CV search.
     V's training set is only ~25 runs × ~5 pilots = ~1250 windows.
     A 15-channel input × 128-hidden BiLSTM has too many free parameters
     and overfits to spurious motion-correlated patterns.
     The candidate combinations now include smaller, focused sets so the
     inner CV can select the right complexity for each vehicle.

  3. Per-fold architecture size: cnn_channels is reduced to 64 (from 96).
     Fewer CNN filters = less overfitting on the small pilot count while
     still capturing the multi-scale temporal patterns.

  4. Cosine LR with warm restarts (CosineAnnealingWarmRestarts, T_0=10)
     replaces the one-shot cosine schedule. Warm restarts help the model
     escape sharp local minima and find flatter solutions that generalise
     better across pilots.

THRESHOLD CALIBRATION (kept from v3, now more effective)
  5. Per-fold threshold calibration via balanced-accuracy grid search is
     retained. Because the class weights now correct the model's raw
     probability outputs, calibrated thresholds will be closer to 0.5
     and more stable across folds.

OUTPUTS (extended)
  6. Bias matrix printed to console and saved as PNG + JSON for every
     vehicle. The threshold used is clearly reported.
  7. Candidate feature-set accuracy is saved per vehicle, with a plot
     showing all tested input/output feature types and clearly marking
     the final configuration used for the model output.
"""
...
class TrainConfig:
    ...
    # Focused combinations — small sets kept so V can find its best one
    input_combinations: Tuple[Tuple[str, ...], ...] = (
        # ── baselines (often best for A; competitive for P) ───────────────
        ("e", "u"),
        ("e", "u", "de", "du"),
        ("e", "u", "de", "du", "d2e", "d2u"),
        # ── spectral only ─────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# FIX 1: Dynamic class-balanced loss
# ─────────────────────────────────────────────────────────────────────────────
...
def compute_class_weights(rows: Sequence[Dict[str, Any]],
    ...
    """
    Compute inverse-frequency class weights from a list of window rows.
    weight_c = n_total / (num_classes * n_c)

    If class 1 (motion) has 60 % of windows and class 0 has 40 %:
        weight_0 = 1 / (2 * 0.4) = 1.25
        weight_1 = 1 / (2 * 0.6) = 0.833
    This exactly cancels window-count imbalance in the gradient.
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# Threshold calibration (kept from v3)
# ─────────────────────────────────────────────────────────────────────────────
...
def find_optimal_threshold(y_true: np.ndarray, y_prob: np.ndarray,
    ...
    for t in np.linspace(0.1, 0.9, grid_points):
...
class ProgressTracker:
    ...
    def _msg(self, prefix: str) -> str:
        ...
        return (
            f"[{prefix}] {min(self.completed_trainings+1,self.total_trainings)}/{self.total_trainings}"
            f" | {frac*100:6.2f}% | ep {self.current_epoch}/{self.current_planned_epochs}"
...
def format_input_vars_label(input_vars: Sequence[str]) -> str:
    return " + ".join(str(v) for v in input_vars)


def shorten_input_vars_label(input_vars: Sequence[str], max_len: int = 48) -> str:
    label = format_input_vars_label(input_vars)
    return label if len(label) <= max_len else label[:max_len - 3] + "..."


def make_candidate_key(input_vars: Sequence[str], window_size: int) -> str:
    return f"{format_input_vars_label(input_vars)} | w={int(window_size)}"


def aggregate_candidate_selection_counts(outer_fold_rows: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    counts: DefaultDict[Tuple[Tuple[str, ...], int], int] = defaultdict(int)
    for row in outer_fold_rows:
        ivars = tuple(str(row["selected_input_vars"]).split(","))
        wsize = int(row["selected_window_size"])
        counts[(ivars, wsize)] += 1

    out = []
    for (ivars, wsize), n in sorted(counts.items(), key=lambda x: (-x[1], x[0][1])):
        out.append({
            "input_vars": ",".join(ivars),
            "input_vars_label": format_input_vars_label(ivars),
            "window_size": int(wsize),
            "times_used_in_final_outer_models": int(n),
        })
    return out
...
def normalize_run_signals(run: Dict[str, Any],
    ...
    # Mean-centre (preserve amplitude differences)
    ...
    # Run-level log-std (amplitude marker)
...
def _first_key(npz_obj: Any, candidates: Sequence[str], required: bool = True) -> Optional[str]:
    for k in candidates:
        if k in npz_obj.files: return k
    if required: raise KeyError(f"None of {tuple(candidates)} found in {npz_obj.files}")
    return None
...
def resolve_save_dir(config: TrainConfig, script_dir: Path) -> Path:
    if config.save_dir in (None, ""):
        return script_dir / "results_lstm_v4"
...
# ─────────────────────────────────────────────────────────────────────────────
# Architecture: Multi-scale CNN + BiLSTM + Attention  (cnn_channels=64, FIX 3)
# ─────────────────────────────────────────────────────────────────────────────
...
def _should_val(epoch, max_epochs, vci):
    vci = max(1, int(vci))
    return epoch == 1 or epoch == max_epochs or epoch % vci == 0
...
def _make_criterion(train_rows: Sequence[Dict[str, Any]],
                    device: torch.device) -> nn.CrossEntropyLoss:
    weights = compute_class_weights(train_rows, num_classes=2, device=device)
...
def fit_lstm(train_rows, val_rows, run_lookup, input_vars, config, device,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer, T_0=max(1, min(10, max_epochs // 3)), T_mult=1, eta_min=1e-5)
    criterion = _make_criterion(train_rows, device)
    ...
    opt_threshold = 0.5
    if return_threshold:
        yt2, _, yp2, _ = predict_loader(model, val_loader, device, config.use_amp)
        opt_threshold = find_optimal_threshold(yt2, yp2)

    return model, history, best_epoch, best_val_acc, opt_threshold
...
def fit_lstm_fixed_epochs(train_rows, run_lookup, input_vars, config, device,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer, T_0=max(1, min(10, planned // 3)), T_mult=1, eta_min=1e-5)
    criterion = _make_criterion(train_rows, device)
    scaler = torch.amp.GradScaler("cuda", enabled=config.use_amp) if device.type == "cuda" else None
    train_loader = make_loader(train_rows, run_lookup, input_vars, config.batch_size, True, config, device)
...
def run_inner_cv_hyperparameter_search(train_pilot_ids, vehicle_type, config, device,
        ...
        for ivars, wsize in candidate_keys:
            ...
            for vp in train_pilot_ids:
                ...
                if not tr or not vr or len({int(r["y"]) for r in tr}) < 2:
                    failed = True; break
                _, _, be, bva, _ = fit_lstm(tr, vr, run_lookup, ivars, config, device,
                    tracker=tracker,
    ...
    for ivars, wsize in selected_keys:
        ...
        cand_folds: List[Dict[str, Any]] = []; failed = False; thresholds: List[float] = []
        for vp in train_pilot_ids:
            tr = collect_rows_for_pilots(ce, [p for p in train_pilot_ids if p != vp])
            vr = collect_rows_for_pilots(ce, [vp])
            if not tr or not vr or len({int(r["y"]) for r in tr}) < 2:
                failed = True; break
            _, _, be, bva, thresh = fit_lstm(tr, vr, run_lookup, ivars, config, device,
                tracker=tracker,
        ...
        summary_rows.append({
            ...
            "mean_opt_threshold":     float(np.mean(thresholds)),
...
def run_vehicle_experiment(runs_for_vehicle, vehicle_type, config, device, tracker=None):
    ...
    tuning_fold_rows: List[Dict[str, Any]] = []
    ...
    global_opt_threshold: float = 0.5
    ...
    if config.global_tune_once_per_vehicle:
        ...
        best_row, all_summary, all_fold_rows = run_inner_cv_hyperparameter_search(
        ...
        global_opt_threshold = float(best_row.get("mean_opt_threshold", 0.5))
        print(f"  → window={global_best_wsize}, vars={','.join(global_best_ivars)}, "
              f"epochs={global_best_epochs}, threshold={global_opt_threshold:.3f}")
        ...
        for row in all_summary:
            tuning_summary_rows.append({
                **row,
                "input_vars_label": format_input_vars_label(str(row["input_vars"]).split(",")),
                "candidate_key": make_candidate_key(str(row["input_vars"]).split(","), int(row["window_size"])),
                "selected_for_final_output": (
                    str(row["input_vars"]) == ",".join(global_best_ivars)
                    and int(row["window_size"]) == int(global_best_wsize)
                ),
            })
    ...
    for test_pilot in pilot_ids:
        ...
        if config.global_tune_once_per_vehicle:
            ...
            best_epochs = global_best_epochs; opt_threshold = global_opt_threshold
        else:
            filtered = {k: v for k, v in candidate_cache_all.items()
            ...
            opt_threshold = float(best_row.get("mean_opt_threshold", 0.5))

            if not tuning_summary_rows:
                tuning_summary_rows = []
                for row in all_summary:
                    tuning_summary_rows.append({
                        **row,
                        ...
                        "candidate_key": make_candidate_key(str(row["input_vars"]).split(","), int(row["window_size"])),
                        "selected_for_final_output": (
                            str(row["input_vars"]) == ",".join(best_ivars)
                            and int(row["window_size"]) == int(best_wsize)
                        ),
                    })
        ...
        for row in window_rows:
            row.update({"outer_test_pilot": test_pilot, "selected_input_vars": ",".join(best_ivars),
                        "selected_input_vars_label": format_input_vars_label(best_ivars),
        ...
        outer_fold_rows.append({
            ...
            "selected_num_epochs": best_epochs, "opt_threshold": opt_threshold,
    ...
    final_selection_counts = aggregate_candidate_selection_counts(outer_fold_rows)
    ...
    return {
        ...
        "tuning_summary_rows": tuning_summary_rows,
        "tuning_fold_rows": tuning_fold_rows,
        "final_selection_counts": final_selection_counts,
        "global_selected_input_vars": list(global_best_ivars) if global_best_ivars is not None else None,
        "global_selected_window_size": int(global_best_wsize) if global_best_wsize is not None else None,
        "global_selected_num_epochs": int(global_best_epochs) if global_best_epochs is not None else None,
        "global_selected_threshold": float(global_opt_threshold),
    }
...
def _save_plot_cm(cm, path, title):
    ...
    fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04); fig.tight_layout(); fig.savefig(path, dpi=160); plt.close(fig)
...
def _save_plot_bias(bm: Dict[str, Any], path: Path, vehicle_type: str) -> None:
    ...
    ax = axes[0]
    ax.bar(["No motion","Motion"], [bm["accuracy_no_motion"],bm["accuracy_motion"]],
    ...
    for i, v in enumerate([bm["accuracy_no_motion"],bm["accuracy_motion"]]):
        ax.text(i, min(0.97, v+0.02), f"{v:.2f}", ha="center", fontsize=11)
    ax2 = axes[1]
    ...
    for i, v in enumerate([bm["false_positive_rate"],bm["false_negative_rate"]]):
        ax2.text(i, min(0.97, v+0.02), f"{v:.2f}", ha="center", fontsize=11)
...
def _save_plot_overall(summary_rows, path):
    ...
    ax.set_title("Overall accuracy by vehicle type (v4)")
...
def _save_plot_candidate_accuracy(candidate_rows, selected_input_vars, selected_window_size, path, vehicle_type):
    if plt is None or not candidate_rows:
        return

    rows = sorted(
        candidate_rows,
        key=lambda r: (-float(r["mean_inner_val_accuracy"]), float(r["std_inner_val_accuracy"]), int(r["window_size"]))
    )
    ...
    selected_input_vars_str = ",".join(selected_input_vars) if isinstance(selected_input_vars, (list, tuple)) else str(selected_input_vars)

    for i, row in enumerate(rows):
        ivars = tuple(str(row["input_vars"]).split(","))
        wsize = int(row["window_size"])
        labels.append(shorten_input_vars_label(ivars, max_len=36) + f"\n[w={wsize}]")
        vals.append(float(row["mean_inner_val_accuracy"]))

        if str(row["input_vars"]) == selected_input_vars_str and wsize == int(selected_window_size):
            final_idx = i

    fig_w = max(10, min(20, 0.9 * len(rows)))
    ...
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("Mean inner-CV accuracy")
    ax.set_title(f"Candidate accuracy by output/input type — {vehicle_type}")
    ax.axhline(0.8, color="grey", linestyle="--", linewidth=0.8)
    ...
    if final_idx is not None:
        bars[final_idx].set_edgecolor("black")
        bars[final_idx].set_linewidth(2.5)
        ax.text(
            final_idx, max(0.05, vals[final_idx] - 0.08),
            "USED FOR FINAL OUTPUT",
            ha="center", va="top", fontsize=9, fontweight="bold", rotation=90
        )
...
def save_vehicle_results(result, save_dir):
    ...
    write_csv(vd / "candidate_results.csv", result.get("tuning_summary_rows", []))
    write_csv(vd / "candidate_fold_results.csv", result.get("tuning_fold_rows", []))
    write_csv(vd / "final_selection_summary.csv", result.get("final_selection_counts", []))
    ...
    if result.get("tuning_summary_rows") and result.get("global_selected_input_vars") is not None:
        _save_plot_candidate_accuracy(
            result["tuning_summary_rows"],
            result["global_selected_input_vars"],
            result["global_selected_window_size"],
            vd / "candidate_accuracy_by_output_type.png",
            str(result["vehicle_type"]),
        )
    ...
    summary = {
        ...
        "final_output_model_selection": {
            "selected_input_vars": result.get("global_selected_input_vars"),
            "selected_input_vars_label": (
                format_input_vars_label(result["global_selected_input_vars"])
                if result.get("global_selected_input_vars") is not None else None
            ),
            "selected_window_size": result.get("global_selected_window_size"),
            "selected_num_epochs": result.get("global_selected_num_epochs"),
            "selected_threshold": result.get("global_selected_threshold"),
        },
        "candidate_results": result.get("tuning_summary_rows", []),
        "final_selection_counts": result.get("final_selection_counts", []),
    }
    ...
    with (vd/"summary.txt").open("w", encoding="utf-8") as f:
        ...
        f.write(f"Calibrated threshold    : {result['mean_threshold']:.3f}\n")
        f.write(f"Overall run accuracy    : {result['test_result']['accuracy']:.3f}\n")

        f.write("\nFINAL OUTPUT MODEL USED\n")
        f.write(f"  Input/output type     : {format_input_vars_label(result['global_selected_input_vars']) if result.get('global_selected_input_vars') else 'N/A'}\n")
        f.write(f"  Window size           : {result.get('global_selected_window_size', 'N/A')}\n")
        f.write(f"  Num epochs            : {result.get('global_selected_num_epochs', 'N/A')}\n")
        f.write(f"  Threshold             : {result.get('global_selected_threshold', 0.5):.3f}\n")

        if result.get("final_selection_counts"):
            f.write("\nHOW OFTEN EACH CONFIGURATION WAS USED IN FINAL OUTER MODELS\n")
            for row in result["final_selection_counts"]:
                f.write(
                    f"  {row['input_vars_label']} | w={row['window_size']} "
                    f"-> used {row['times_used_in_final_outer_models']} time(s)\n"
                )

        if result.get("tuning_summary_rows"):
            f.write("\nTOP CANDIDATES BY INNER-CV ACCURACY\n")
            for row in result["tuning_summary_rows"][:5]:
                selected_flag = "  <-- USED FOR FINAL OUTPUT" if row.get("selected_for_final_output") else ""
                f.write(
                    f"  acc={float(row['mean_inner_val_accuracy']):.3f} ± {float(row['std_inner_val_accuracy']):.3f}"
                    f" | w={int(row['window_size'])}"
                    f" | {row['input_vars_label']}{selected_flag}\n"
                )
    ...
    return {"vehicle_type": result["vehicle_type"],
            ...
            "selected_input_vars": (
                format_input_vars_label(result["global_selected_input_vars"])
                if result.get("global_selected_input_vars") is not None else "N/A"
            ),
            "selected_window_size": result.get("global_selected_window_size")}
...
def print_summary(result):
    ...
    print(f"Vehicle: {result['vehicle_type']}  |  folds={result['num_outer_folds']}  "
          ...
          f"|  threshold={result['mean_threshold']:.3f}")
    if result.get("global_selected_input_vars") is not None:
        print(f"Final output uses: window={result.get('global_selected_window_size')}  "
              f"inputs={format_input_vars_label(result['global_selected_input_vars'])}")
...
def main(config: TrainConfig) -> Dict[str, Any]:
    ...
    if overall_rows:
        ...
        for row in overall_rows:
            print(
                f"  {row['vehicle_type']}: "
                f"accuracy={row['run_level_accuracy']:.3f}  "
                f"threshold={row['mean_threshold']:.3f}  "
                f"window={row.get('selected_window_size', 'N/A')}  "
                f"inputs={row.get('selected_input_vars', 'N/A')}"
            )
        print("=" * 72)
    return all_results
...
if __name__ == "__main__":
    ...
    config = TrainConfig(
        ...
        save_dir=r"C:\\Users\\bramb\\Downloads\\AI_project_simulator\\A02-TAS-Repository\\results_lstm_v5_new",
