...
def build_model(config: TrainConfig, input_size: int, device: torch.device) -> nn.Module:
    model = MultiScaleCNNBiLSTMClassifier(
        ...
        cnn_channels=64,  # FIX 3: constrained to limit overfitting
    ).to(device)
...
# ─────────────────────────────────────────────────────────────────────────────
# Training loops  (FIX 1: class weights; FIX 4: warm restarts; FIX: grad clip)
# ─────────────────────────────────────────────────────────────────────────────
...
def _make_criterion(train_rows: Sequence[Dict[str, Any]],
    ...
    """FIX 1: CrossEntropyLoss with per-fold inverse-frequency class weights."""
...
def fit_lstm(train_rows, val_rows, run_lookup, input_vars, config, device,
    ...
    # FIX 4: cosine annealing with warm restarts
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer, T_0=max(1, min(10, max_epochs // 3)), T_mult=1, eta_min=1e-5)
    # FIX 1: class-balanced loss (recomputed per fold)
...
def fit_lstm_fixed_epochs(train_rows, run_lookup, input_vars, config, device,
    ...
    criterion = _make_criterion(train_rows, device)   # FIX 1
    scaler = torch.amp.GradScaler("cuda", enabled=config.use_amp) if device.type == "cuda" else None
    train_loader = make_loader(train_rows, run_lookup, input_vars, config.batch_size, True, config, device)
...
def run_vehicle_experiment(runs_for_vehicle, vehicle_type, config, device, tracker=None):
    ...
    for test_pilot in pilot_ids:
        ...
        # Log class balance in training split (diagnostic)
        ...
        outer_fold_rows.append({
            ...
            "selected_input_vars": ",".join(best_ivars), "selected_window_size": best_wsize,
...
if __name__ == "__main__":
    ...
    config = TrainConfig(
        ...
        save_dir=r"C:\Users\bramb\Downloads\AI_project_simulator\A02-TAS-Repository\results_lstm_v4",
