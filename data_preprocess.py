...
for subject in range(1, 7):
    for condition in range(1, 7):
        ...
        # Extract the needed arrays
        positions = data_dict["positions"]
        velocities = data_dict["velocities"]
        accelerations = data_dict["accelerations"]
        # Calculate the mean over the various repetitions
        mean_positions = np.mean(positions, axis=0)
        mean_velocities = np.mean(velocities, axis=0)
        mean_accelerations = np.mean(accelerations, axis=0)
        # Update the values dictionary with the mean values
        values[key] = {
