...
"""
Improved LSTM classifier for motion-on vs motion-off in AE2224-I dataset.

KEY IMPROVEMENTS OVER BASELINE (targeting P=53% → higher, V=50% → higher):
───────────────────────────────────────────────────────────────────────────
1. NORMALIZATION  : Replaced full z-score with mean-only centering (no
                   variance normalization). Amplitude differences between
                   motion-on and motion-off conditions are preserved.
                   Per-run global std is saved as a *separate* feature
                   channel ("e_logstd", "u_logstd") so the model can
                   learn from it explicitly.

2. NEW FEATURES   :
   • d2e, d2u  – second derivatives (jerk/acceleration-level signals).
                 These are most informative for P and V conditions where
                 the vestibular cueing operates on position and velocity.
   • eu        – element-wise product e × u. Encodes the open-loop pilot-
                 vehicle correlation, which shifts with motion cueing at
                 mid frequencies (0.5–3 Hz).
   • e_logstd, u_logstd – scalar log-std of the run, broadcast as
                 constant channels. Lets the model compare run-level power.

3. LONGER WINDOWS : Default window range extended to (128, 256, 512).
                   At 100 Hz that is 1.3 – 5.1 s of context, enough to
                   see vestibular-driven frequency-band differences.

4. ARCHITECTURE   : Replaced plain LSTM with CNNBiLSTMClassifier:
   • 3-layer 1-D CNN extracts local temporal patterns before the LSTM.
   • Bidirectional LSTM (sees future context in both directions).
   • Temporal soft-attention collapses the sequence by weighting the
     most discriminative time steps rather than using only the final state.
   • Residual skip connection from CNN output to the attention-weighted
     LSTM output for gradient flow.

5. HYPERPARAMETERS: hidden_size=128, larger windows, more input combos.

Everything else (nested CV, lazy dataset, DataLoader config, CSV/plot
export, progress tracking) is unchanged from the original.
"""
...
class TrainConfig:
    ...
    # ── IMPROVEMENT 3: larger windows give more temporal context ──────────
    window_sizes: Tuple[int, ...] = (128, 256, 512)
    stride_fraction: float = 0.5

    # ── IMPROVEMENT 2: richer input combinations including new features ───
    input_combinations: Tuple[Tuple[str, ...], ...] = (
        ("e", "u"),
        ("e", "u", "de", "du"),
        ("e", "u", "de", "du", "d2e", "d2u"),
        ("e", "u", "eu", "de", "du"),
        ("e", "u", "eu", "de", "du", "d2e", "d2u"),
        ("e", "u", "eu", "de", "du", "d2e", "d2u", "e_logstd", "u_logstd"),
    )

    batch_size: int = 256
    eval_batch_size: int = 2048
    # ── IMPROVEMENT 4: bigger hidden size for the richer architecture ─────
    hidden_size: int = 128
    num_layers: int = 2
    dropout: float = 0.3
...
class ProgressTracker:
    ...
    def _status_message(self, prefix: str) -> str:
        ...
        return (
            ...
            f"{self.total_trainings} | overall {frac * 100:6.2f}% | "
            f"epoch {self.current_epoch}/{self.current_planned_epochs} | "
...
def confusion_matrix_np(y_true: np.ndarray, y_pred: np.ndarray, labels: Sequence[int] = (0, 1)) -> np.ndarray:
    y_true = np.asarray(y_true, dtype=int); y_pred = np.asarray(y_pred, dtype=int)
    labels = list(labels); index = {label: i for i, label in enumerate(labels)}
...
def mean_center(x: np.ndarray) -> np.ndarray:
    """
    IMPROVEMENT 1: Mean-center only (do NOT divide by std).
    Preserves amplitude differences between motion-on / motion-off runs.
    """
    x = np.asarray(x, dtype=np.float32)
...
def normalize_run_signals(run: Dict[str, Any]) -> Dict[str, Any]:
    """
    IMPROVEMENT 1 + 2:
    • Mean-center e and u instead of full z-score  → preserves amplitude.
    • Compute derivatives with safe_zscore (scale-free shape features).
    • Compute second derivatives d2e, d2u            → new feature channels.
    • Compute element-wise product eu = e*u           → new feature channel.
    • Broadcast log(std(e)) and log(std(u)) as constant channels so the
      model can compare power levels across windows.
    """
    ...
    # ── Mean-center (not z-score) for e and u ────────────────────────────
    out["e"] = mean_center(raw_e)
    out["u"] = mean_center(raw_u)

    # ── Z-score derivatives (shape matters, not amplitude) ────────────────
    out["de"] = safe_zscore(de_raw)
    out["du"] = safe_zscore(du_raw)

    # ── Second derivatives (new) ──────────────────────────────────────────
    d2e_raw = derivative(de_raw, time=time_vec, fs=fs)
    d2u_raw = derivative(du_raw, time=time_vec, fs=fs)
    out["d2e"] = safe_zscore(d2e_raw)
    out["d2u"] = safe_zscore(d2u_raw)

    # ── Cross-signal interaction e × u (new) ─────────────────────────────
    out["eu"] = safe_zscore(out["e"] * out["u"])

    # ── Log-std amplitude channels (broadcast scalars, new) ───────────────
    log_std_e = float(np.log1p(np.std(raw_e) + 1e-8))
    log_std_u = float(np.log1p(np.std(raw_u) + 1e-8))
    n = len(raw_e)
    out["e_logstd"] = np.full(n, log_std_e, dtype=np.float32)
    out["u_logstd"] = np.full(n, log_std_u, dtype=np.float32)
...
def _first_available_key(npz_obj: Any, candidates: Sequence[str], required: bool = True) -> Optional[str]:
    for key in candidates:
        if key in npz_obj.files:
            return key
    if required:
        raise KeyError(f"None of the keys {tuple(candidates)} found. Available: {npz_obj.files}")
    return None
...
def resolve_data_dir(config: TrainConfig, script_dir: Path) -> Path:
    ...
    raise FileNotFoundError(f"Could not find data directory. Checked:\n{checked}")


def resolve_save_dir(config: TrainConfig, script_dir: Path) -> Path:
    if config.save_dir in (None, ""):
        return script_dir / "results_lstm_improved"
...
def load_runs_from_npz_dir(data_dir, config: TrainConfig, min_run_length: int = 1) -> List[Dict[str, Any]]:
    ...
    if not npz_files:
        raise FileNotFoundError(f"No .npz files matched pattern in {data_path}.")
    ...
    for npz_file in npz_files:
        ...
        with np.load(npz_file, allow_pickle=True) as npz_obj:
            ...
            if len(e_runs) != len(u_runs):
                raise ValueError(f"{npz_file.name}: e/u run count mismatch")
            if t_key is None:
                ...
                if len(raw_t_runs) == 1 and len(e_runs) > 1:
                    ...
                    raise ValueError(f"{npz_file.name}: t run count mismatch")
...
class LazyWindowDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        start = int(row["start_idx"]); end = int(row["end_idx"])
...
# ──────────────────────────────────────────────────────────────────────────────
# IMPROVEMENT 4: CNN-BiLSTM-Attention classifier
# ──────────────────────────────────────────────────────────────────────────────

class TemporalAttention(nn.Module):
    """Soft temporal attention over LSTM hidden states."""
...
class CNNBiLSTMClassifier(nn.Module):
    ...
    1. 1-D CNN front-end  : three convolutional layers with kernel sizes 7, 5,
                            3 extract local temporal patterns (e.g. frequency
                            content) before the recurrent layers see the data.
    2. Bidirectional LSTM : processes the CNN output in both time directions,
                            giving every time step access to future context.
    3. Temporal attention : learns which time steps are most discriminative for
                            motion-on vs motion-off rather than using only the
                            final hidden state.
    4. Residual skip      : mean-pools the CNN output and adds it to the
                            attention vector before classification, improving
                            gradient flow to shallow layers.
    """
    ...
    def __init__(
        ...
        self.cnn = nn.Sequential(
            nn.Conv1d(input_size, cnn_channels, kernel_size=7, padding=3),
        ...
        lstm_out_size = hidden_size * 2  # bidirectional doubles the output

        # ── Temporal attention ─────────────────────────────────────────────
        self.attention = TemporalAttention(lstm_out_size)

        # ── Residual projection of CNN pooled output ───────────────────────
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        cnn_out = self.cnn(x.permute(0, 2, 1))          # (B, C, L)
        cnn_seq = cnn_out.permute(0, 2, 1)               # (B, L, C) → LSTM input
        ...
        # Residual: mean-pool CNN output and project to same size
        cnn_pooled = cnn_seq.mean(dim=1)                 # (B, C)
        residual = self.residual_proj(cnn_pooled)        # (B, H*2)

        fused = context + residual                       # (B, H*2)
        return self.classifier(fused)


def build_model(config: TrainConfig, input_size: int, device: torch.device) -> nn.Module:
    """Returns the improved CNNBiLSTMClassifier."""
    cnn_channels = min(128, max(32, config.hidden_size))
...
# ──────────────────────────────────────────────────────────────────────────────
# Training loops (unchanged logic, same API)
# ──────────────────────────────────────────────────────────────────────────────
...
def fit_lstm(train_rows, val_rows, run_lookup, input_vars, config, device, tracker=None, training_label="",
             max_epochs_override=None, patience_override=None, val_check_interval_override=None):
    model = build_model(config, input_size=len(input_vars), device=device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=config.learning_rate, weight_decay=config.weight_decay)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_epochs_override or config.max_epochs, eta_min=1e-5)
    ...
    if tracker is not None:
        tracker.start_training(training_label or f"train={len(train_rows)}", max_epochs)
...
def fit_lstm_fixed_epochs(train_rows, run_lookup, input_vars, config, device, num_epochs, tracker=None, training_label=""):
    model = build_model(config, input_size=len(input_vars), device=device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=config.learning_rate, weight_decay=config.weight_decay)
    ...
    scaler = torch.amp.GradScaler("cuda", enabled=config.use_amp) if device.type == "cuda" else None
    train_loader = make_loader(train_rows, run_lookup, input_vars, config.batch_size, True, config, device)
    planned_epochs = max(1, int(num_epochs)); history: List[Dict[str, Any]] = []
...
def aggregate_run_predictions(pred_rows):
    ...
    for row in pred_rows:
        key = (str(row["vehicle_type"]), str(row["pilot_id"]), str(row["repetition_id"]), str(row["run_id"]), int(row["true_label"]))
    ...
    for (vehicle_type, pilot_id, repetition_id, run_id, true_label), scores in grouped.items():
        ...
        out.append({
            "vehicle_type": vehicle_type, "pilot_id": pilot_id, "repetition_id": repetition_id,
            "run_id": run_id, "true_label": true_label,
            "pred_label": int(mean_conf >= 0.5), "motion_on_confidence": mean_conf,
...
# ──────────────────────────────────────────────────────────────────────────────
# Inner CV & outer fold logic (unchanged)
# ──────────────────────────────────────────────────────────────────────────────
...
def run_inner_cv_hyperparameter_search(train_pilot_ids, vehicle_type, config, device, run_lookup, candidate_cache, tracker=None):
    ...
    if len(train_pilot_ids) < 2:
        raise ValueError("Need at least two pilots in outer-training split for inner CV.")
        ...
        for input_vars, window_size in candidate_keys:
            ...
            for val_pilot in train_pilot_ids:
                ...
                _, _, best_epoch, best_val_acc = fit_lstm(
                    train_rows=train_rows, val_rows=val_rows, run_lookup=run_lookup,
                    input_vars=input_vars, config=config, device=device, tracker=tracker, training_label=label,
                    max_epochs_override=config.stage1_epochs, patience_override=config.stage1_patience,
                    val_check_interval_override=max(2, config.val_check_interval),
                )
                accs.append(float(best_val_acc)); epochs.append(int(best_epoch))
    ...
    for input_vars, window_size in selected_keys:
        ...
        candidate_fold_rows: List[Dict[str, Any]] = []; failed = False
        for val_pilot in train_pilot_ids:
            inner_train_pilots = [p for p in train_pilot_ids if p != val_pilot]
            train_rows = collect_rows_for_pilots(candidate_entry, inner_train_pilots)
            val_rows   = collect_rows_for_pilots(candidate_entry, [val_pilot])
            ...
            candidate_fold_rows.append({
                ...
                "best_epoch": int(best_epoch), "best_val_accuracy": float(best_val_acc),
            })
        if failed or not candidate_fold_rows: continue
        fold_rows.extend(candidate_fold_rows)
        accs = [r["best_val_accuracy"] for r in candidate_fold_rows]
        epochs = [r["best_epoch"] for r in candidate_fold_rows]
        summary_rows.append({
            "vehicle_type": vehicle_type, "input_vars": ",".join(input_vars),
            "window_size": int(window_size),
...
def run_vehicle_experiment(runs_for_vehicle, vehicle_type, config, device, tracker=None):
    ...
    if len(pilot_ids) < 3:
        raise ValueError(f"Vehicle {vehicle_type}: need ≥3 pilots for nested CV.")
    ...
    if bool(config.global_tune_once_per_vehicle):
        print(f"\nVehicle {vehicle_type}: running shared hyperparameter search")
        filtered_candidate_cache = {
            k: v for k, v in candidate_cache_all.items()
            if all(v["rows_by_pilot"].get(p, []) for p in pilot_ids)
        }
        if not filtered_candidate_cache:
            raise ValueError(f"Vehicle {vehicle_type}: no candidate had data for every pilot.")
        best_row, _, _ = run_inner_cv_hyperparameter_search(
            pilot_ids, vehicle_type, config, device, run_lookup_all, filtered_candidate_cache, tracker)
        ...
        print(f"Vehicle {vehicle_type}: selected → window={global_best_window_size}, "
    ...
    for test_pilot in pilot_ids:
        ...
        if len({int(r["label"]) for r in outer_train_runs}) < 2:
            print(f"Skipping {vehicle_type}/{test_pilot}: single class in outer train"); continue
        ...
        if bool(config.global_tune_once_per_vehicle):
            ...
            best_row, _, _ = run_inner_cv_hyperparameter_search(
                outer_train_pilots, vehicle_type, config, device, run_lookup_all, filtered_candidate_cache, tracker)
        ...
        final_model, _ = fit_lstm_fixed_epochs(
            train_rows=train_rows, run_lookup=run_lookup_all, input_vars=best_input_vars,
            config=config, device=device, num_epochs=selected_num_epochs, tracker=tracker,
    ...
    if not combined_run_rows:
        raise ValueError(f"No valid outer folds for vehicle {vehicle_type}.")
...
def print_compute_summary(device, config):
    ...
    if device.type == "cuda":
        ...
        print(f"Mixed precision       : {config.use_amp}  |  TF32: {config.allow_tf32}")
    print(f"Architecture          : CNNBiLSTMClassifier (improved)")
    print(f"hidden_size           : {config.hidden_size}  (bidirectional → {config.hidden_size*2} LSTM output)")
    print(f"Training batch size   : {config.batch_size}")
    print(f"Window sizes          : {config.window_sizes}")
    print(f"Input combinations    : {len(config.input_combinations)}")
    print("=" * 88)
...
def print_summary(result):
    ...
    if result["outer_fold_rows"]:
        ...
        print(f"{'Pilot':<12}{'Accuracy':>12}{'Window':>10}{'Inputs':>28}{'Epochs':>8}")
        for row in result["outer_fold_rows"]:
            print(f"{str(row['outer_test_pilot']):<12}"
                  f"{float(row['run_level_accuracy']):>12.3f}"
                  f"{int(row['selected_window_size']):>10}"
                  f"{str(row['selected_input_vars']):>28}"
...
def main(config: TrainConfig) -> Dict[str, Any]:
    ...
    for vehicle_type in sorted(runs_by_vehicle.keys()):
        ...
        pilot_ids = get_unique_pilot_ids(vehicle_runs)
        if len(labels) < 2:
            print(f"Skipping {vehicle_type}: one class only"); continue
        if len(pilot_ids) < 3:
            print(f"Skipping {vehicle_type}: fewer than 3 pilots"); continue
    ...
    if overall_rows:
        ...
        print("No vehicle experiments completed.")
    return all_results
...
if __name__ == "__main__":
    ...
    config = TrainConfig(
        ...
        save_dir=r"C:\\Users\\bramb\\Downloads\\AI_project_simulator\\A02-TAS-Repository\\results_lstm_improved",

        # ── IMPROVEMENT 3: wider range of window sizes ─────────────────────
        window_sizes=(128, 256, 512),
        stride_fraction=0.5,

        # ── IMPROVEMENT 2: all new feature combinations ────────────────────
        input_combinations=(
            ("e", "u"),
            ("e", "u", "de", "du"),
            ("e", "u", "de", "du", "d2e", "d2u"),
        ...
        # ── IMPROVEMENT 4: bigger model for richer architecture ────────────
        ...
        # ── AdamW + cosine LR (replaces Adam + flat LR) ───────────────────
        learning_rate=5e-4,
        weight_decay=1e-4,
        max_epochs=40,
        patience=8,
        ...
        # ── Stage-1 pruning (unchanged logic, just adjusted counts) ────────
        stage1_enabled=True,
        stage1_epochs=8,
        stage1_patience=3,
        stage1_top_k=4,
